...
@Slf4j
@AllArgsConstructor
public class UpdateTempChartForRollback extends MessageHandler<HelmReleaseLifecycleMessage> {

    private MessageUtility utility;

    @Override
    public void handle(MessageHandlingContext<HelmReleaseLifecycleMessage> context) {
        LOGGER.info("Handling updating temp chart for rollback");
        utility.updateTempChartForRollback(context);
