...
import com.ericsson.vnfm.orchestrator.messaging.handlers.terminate.DeletePvc;
...
@Component
@Slf4j
public class TerminateOperation extends AbstractLifeCycleOperationProcessor {

    public static final Pattern REGEX_NAMESPACE_NOT_FOUND = Pattern.compile("(.)*Namespace (.)* not found in cluster(.)*");
    ...
    public Conditions getConditions() {
        return new Conditions(MessagingLifecycleOperationType.TERMINATE.toString(), HelmReleaseLifecycleMessage.class);
    }
    ...
    public MessageHandlingConfiguration<HelmReleaseLifecycleMessage> configureCompleted() {
        ...
        return new MessageHandlingConfiguration<HelmReleaseLifecycleMessage>()
                ...
                .andThen(new DeletePvc(utility, lifeCycleManagementHelper))
                .andThenOrElse(new AnyFailedReleases(), persistAndDeleteIdentifierSubflow)
                ...
                .andThenOrElse(new DeleteNamespace(utility, databaseInteractionService, lifeCycleManagementHelper), persistAndDeleteIdentifierSubflow)
    ...
    public void failed(final HelmReleaseLifecycleMessage message) {
        if (message.getMessage() != null && isReleaseNameOrNamespaceNotFound(message.getMessage())) {
    ...
    public static boolean isReleaseNameOrNamespaceNotFound(String errorMessage) {
    ...
    public MessageHandlingConfiguration<HelmReleaseLifecycleMessage> configureFailed() {
        ...
        return new MessageHandlingConfiguration<HelmReleaseLifecycleMessage>()
                ...
                .andThen(new RedirectFromTerminateToCcvp(changeVnfPackageOperation, databaseInteractionService))
                .andThen(new RedirectFromTerminateToRollback(rollbackOperation, databaseInteractionService))
