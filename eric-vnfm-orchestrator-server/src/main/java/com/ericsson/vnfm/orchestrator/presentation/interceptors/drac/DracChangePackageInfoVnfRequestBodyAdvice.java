 ...
 * to {@link VnfInstancesControllerImpl#changeVnfInstancePackageInfoById(String, String, String, String, ChangePackageInfoVnfRequest)}
 * or
 * {@link VnfInstancesControllerImpl#changeVnfInstancePackageInfoById(String, String, String, String, MultipartFile, ChangePackageInfoVnfRequest)}
 * as body.
 */
@ControllerAdvice
@Slf4j
public class DracChangePackageInfoVnfRequestBodyAdvice extends RequestBodyAdviceAdapter {

    private static final String CHANGE_PACKAGE_INFO_VNF_METHOD_NAME = "changeVnfInstancePackageInfoById";

    private final PackageService packageService;
    private final DracService dracService;

    public DracChangePackageInfoVnfRequestBodyAdvice(final DracService dracService,
    ...
    public boolean supports(final MethodParameter methodParameter,
        ...
        return isVnfInstanceController(methodParameter) && isChangePackageInfoVnfMethodName(methodParameter);
    }
    ...
    private static boolean isChangePackageInfoVnfMethodName(final MethodParameter methodParameter) {
        final Method method = methodParameter.getMethod();
        final String methodName = method != null ? method.getName() : null;

        return Objects.equals(methodName, CHANGE_PACKAGE_INFO_VNF_METHOD_NAME);
    }
    ...
    private void checkUserRoleIfNecessary(final Object body) {
        ...
        if (body instanceof ChangePackageInfoVnfRequest) {
            changePackageInfoVnfRequest = (ChangePackageInfoVnfRequest) body;
        } else {
            changePackageInfoVnfRequest = Utility.parseJsonIgnoreUnknown(body.toString(), ChangePackageInfoVnfRequest.class);
        }

        final var nodeType = lookupNodeType(changePackageInfoVnfRequest);
    ...
    private Optional<String> lookupNodeType(final ChangePackageInfoVnfRequest changePackageInfoVnfRequest) {
        try {
            final PackageResponse packageInfo = packageService
                    .getPackageInfo(changePackageInfoVnfRequest.getVnfdId());
            ...
            LOGGER.warn(format(GET_PACKAGE_ERROR_MESSAGE, changePackageInfoVnfRequest.getVnfdId()), e);
