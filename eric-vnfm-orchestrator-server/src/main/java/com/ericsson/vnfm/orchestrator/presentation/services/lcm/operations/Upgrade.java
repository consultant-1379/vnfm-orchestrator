...
public class Upgrade implements Command {
    ...
    public void execute(final LifecycleOperation operation, final HelmChart helmChart, final boolean isDowngradeOperation) {
        LOGGER.info("Starting Upgrade command");
        ...
        triggerUpgradeOperation(operation, helmChart, additionalParams, valuesFile);
    }
    ...
    private Map<String, Object> getOperationAdditionalParams(final LifecycleOperation operation, final boolean isDowngradeOperation) {
        ...
        if (isDowngradeOperation || StringUtils.isNotBlank(operation.getUpgradePattern())) {
    ...
    private void triggerUpgradeOperation(LifecycleOperation operation,
                                         HelmChart helmChart,
        ...
        final WorkflowRoutingResponse workflowRoutingResponse = StringUtils.isEmpty(operation.getUpgradePattern()) ?
                workflowRoutingService.routeToEvnfmWfsFakeUpgrade(originalInstance, operation, helmChart, additionalParams, toValuesFile) :
                workflowRoutingService.routeToEvnfmWfsUpgrade(operation, helmChart, additionalParams, toValuesFile);
