...
public class PostRollbackOperationPositiveBase extends AbstractDbSetupTest {
    ...
    @BeforeEach
    public void setUp() {
        given(lifeCycleManagementService.executeRequest(any(), anyString(), any(InstantiateVnfRequest.class), anyString(), anyMap())).willReturn(
                "5f43fb8e-1316-468a-9f9c-b375e5d82094");
        given(lifeCycleManagementService.executeRequest(any(), anyString(), any(InstantiateVnfRequest.class), anyString(), anyMap())).willReturn(
                "5f43fb8e-1316-468a-9f9c"
                        + "-b375e5d82094");
        doNothing().when(databaseInteractionService).addToOperationsInProgressTable(anyString(), any());
