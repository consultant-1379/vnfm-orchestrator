...
```
given:
  client requests to view cluster configs on 2nd page (page numbers start from 1) with page size of 2 and in descending order sorted by name
when:
...
""")
    request {
        ...
        urlPath("/vnflcm/v1/clusterconfigs") {
            queryParameters {
                parameter 'page': '2'
                parameter 'size': '2'
                parameter 'sort': value(consumer(regex(/(name,desc)/)), producer("name,desc"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file('pageTwoClusterConfigsDesc.json'))
        bodyMatchers {
