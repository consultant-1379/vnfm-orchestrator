 ...
 ******************************************************************************/
package contracts.api.getResources.podstatus.negative.noPodsFound
...
""")
    request {
        method 'GET'
        url "/vnflcm/v1/resources/${value(consumer(anyNonEmptyString()), producer('test'))}NOPODSFOUND/pods"
    }
    response {
        status NOT_FOUND()
        body(
                """
                    {
                        ...
                        "status":404,
                        "detail":"No pods found for instance ${fromRequest().path(3)}",
